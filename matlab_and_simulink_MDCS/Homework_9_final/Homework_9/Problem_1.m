...
R_0 = 1;
nu_1 = 0;
S_star_zero_1 = abs(e_r / (K_d*R_0)); % |S_star_zero| < 0.15

%S3
e_da = 1.5e-2;
nu_2 = 0;
S_star_zero_2 = abs(e_da / (K_pn*D_a0)); % |S_star_zero| < 0.2182

%S4
e_dp = 5e-4;
M_S_LF = e_dp / a_p;
M_S_LF_dB = 20*log10(M_S_LF); % M_S_LF_dB = -32.0412dB

%S5
e_ds = 5e-4;
M_T_HF = e_ds * G_s / a_s;
M_T_HF_dB = 20*log10(M_T_HF); % M_T_HF_dB = -46.0206dB

%% Transient requirements
%S8
s_hat = 0.1;
zeta = abs(log(s_hat)) / (sqrt(pi^2 + log(s_hat)^2)); % zeta = 0.5912

%S6
t_r = 3;
w_n_tr = 1 / (t_r * sqrt(1 - zeta^2)) * (pi - acos(zeta)); % w_n > 0.9106rad/s

%S7
t_s5 = 12;
w_n_ts5 = log(100/5) / (t_s5 * zeta); % w_n > 0.4223rad/s
...
nu = max(nu_1, nu_2); % nu = 0
w_n_min = max(w_n_tr, w_n_ts5); % w_c_min = 0.9106rad/s

S = s * (s + 2*zeta*w_n_min) / (s^2 + 2*zeta*w_n_min*s + w_n_min^2);

%% Weighting funciton W_S_inv design
a = min(S_star_zero_1, S_star_zero_2);
S_star_zero = a*s; 
...
W_1 = zpk(1 / W_S_inv); % Use W_1_inv for second order, otherwise W_S_inv for third order.
...
% Second Order
% G_cmod = minreal(G_cmod * (s+0.006572) / (s+4.94e-06), 1e-3);
% G_cmod = minreal(G_cmod * (1+s/8312), 1e-3)

% Third order
G_cmod = minreal(G_cmod * (s+0.006573) / (s+7.263e-06), 1e-3)
G_cmod = minreal(G_cmod * (1+s/6272), 1e-3)
...
yline(K_d+0.1*K_d, '--r', LineWidth=0.6)
yline(K_d+0.05*K_d, '--m', LineWidth=0.6)
yline(K_d-0.05*K_d, '--m', LineWidth=0.6)
xline(3, '--k', LineWidth=0.6)
xline(12, '--k', LineWidth=0.6)
grid on
