clear all
close all
clc

%% Problem Definition
% Parameter uncertainties
K = [-20, -10];
pole1 = [0.8, 1.2];
pole2 = [1.6, 2.4];
K_pn = mean(K);
p1_n = mean(pole1);
p2_n = mean(pole2);

% Plant 
s = tf('s');
G_pn = K_pn / (s * (1 + s/p1_n) * (1 + s/p2_n));
K_p = dcgain(G_pn*s);
G_s = 10;
G_a = 0.06;
D_a0 = 2.5e-3;
D_p0 = 8.5e-3;
a_p = 0; 
w_p = 0;
a_s = 5e-2;
w_s = 40;

%% Requirements translation
%S1
K_d = 10;
G_f = 1 / (K_d*G_s);

%S2
e_r = 2.5e-1;
R_0 = 1;
nu_1 = 0;
S_star_zero_1 = e_r / (K_d*R_0); % |S_star_zero| < 0.025

%S3
e_da = 1e-2;
nu_2 = 0;
S_star_zero_2 = e_da / (K_p*D_a0); % |S_star_zero| < 0.2667

%S4
e_dp = 0.94;
nu_3 = 1;
h_p = 2;
S_star_zero_3 = e_dp / (factorial(h_p)*D_p0); % |S_star_zero| < 55.2941

%S5
e_ds = 1.6e-5;
M_T_HF = e_ds * G_s / a_s;
M_T_HF_dB = 20*log10(M_T_HF); % M_T_HF_dB = -49.897dB

%% Transient requirements
%S8
s_hat = 0.14;
zeta = abs(log(s_hat)) / (sqrt(pi^2 + log(s_hat)^2)); % zeta = 0.5305

%S6
t_r = 2.5;
w_n_tr = 1 / (t_r * sqrt(1 - zeta^2)) * (pi - acos(zeta)); % w_n > 1.0051rad/s 

%S7
t_s5 = 13;
w_n_ts5 = log(100/5) / (t_s5 * zeta); % w_n > 0.4344rad/s 

S_p = 2 * zeta * sqrt(2 + 4 * zeta^2 + 2 * sqrt(1 + 8 * zeta^2)) /...
    (sqrt(1 + 8 * zeta^2) + 4 * zeta^2 - 1);
T_p = 1 / (2 * zeta * sqrt(1 - zeta^2));

%% Choice of parameters
nu = max(nu_1, max(nu_2, nu_3)); % nu = 1
w_n_min = max(w_n_tr, w_n_ts5); % w_c_min = 1.0051rad/s

S = s * (s + 2*zeta*w_n_min) / (s^2 + 2*zeta*w_n_min*s + w_n_min^2);

%% Weighting funciton W_S_inv design
a = S_star_zero_3;
S_star_zero = a*s^2; 

figure, hold on
bodemag(S, {1e-4, 1e+3}, '--g')
bodemag(S_star_zero, 'g') 
semilogx([1e-4, 1e+3], [20*log10(S_p), 20*log10(S_p)], 'r')

W_S_inv = a * s^2;
% bodemag(W_S_inv)

% Real pole
p1 = 0.019;
W_S_inv = W_S_inv * 1 / (1 + s/p1);
% bodemag(W_S_inv)

% Real zero and complex conjugate poles
z2 = 0.55;
p2 = sqrt(S_p*z2/(a*p1));
zeta = 0.8;
W_S_inv = W_S_inv * (1 + s/z2) / (1 + 2*zeta*s/p2 + s^2/p2^2);
bodemag(W_S_inv, {1e-4, 1e+3})

grid on, hold off

%% Weighting funciton W_T_inv design
p1 = w_s / 10^((20*log10(T_p)-M_T_HF_dB)/40);
zeta = 0.72;
W_T_inv = T_p / (1 + 2*zeta*s/p1 + s^2/p1^2);

figure, hold on
bodemag(W_T_inv, {1e-3, 1e+3}, 'g')
semilogx([w_s, 1e+3], [M_T_HF_dB, M_T_HF_dB], 'r', LineWidth=1.5)
semilogx([w_s, w_s], [M_T_HF_dB, 0], 'r', LineWidth=1.5)
semilogx([1e-3, 1e+3], [20*log10(T_p), 20*log10(T_p)], 'r')
grid on, hold off

%% Weighting function W_u
n = 10;
x_K = linspace(K(1), K(2), n);
x_p1 = linspace(pole1(1), pole1(2), n);
x_p2 = linspace(pole2(1), pole2(2), n);
w = logspace(-3, 3, 1000);
mG_pnf = freqresp(G_pn, w);
count = 1;
mf = zeros(1000,1);

figure, 
for i = 1:n
    for j = 1:n
        for k = 1:n
            G_p = x_K(i) / (s * (1 + s/x_p1(j)) * (1 + s/x_p2(k)));
            mG_pf = freqresp(G_p, w);
            Delta_mf = abs((mG_pf ./ mG_pnf) - 1);
            mag = squeeze(Delta_mf);
            loglog(w, mag), hold on
            mf = max(mf, mag);
        end
    end
end

loglog(w, mf, '--c', 'LineWidth', 1.5)
magg = vpck(mf(:), w);

figure,
W_u_m = fitmag(magg);
[A,B,C,D] = unpck(W_u_m);
[z,p,k] = ss2zp(A,B,C,D);
W_u_m = zpk(z,p,k);
% W_u_m = ss2tf(A,B,C,D);

%% Robust Control
zeta = 0.6083;
w_c = w_n_min * sqrt(sqrt(1+4*zeta^4)-2*zeta^2);

% W_1
% p1 = 0.019;
% z2 = 0.55;
% p2 = sqrt(S_p*z2/(a*p1));
% zeta = 0.8;
% W_1_inv = a * s^2 * 1 / (1 + s/p1) * (1 + s/z2) / (1 + 2*zeta*s/p2 + s^2/p2^2);
W_1 = zpk(1 / W_S_inv);
W_1mod = zpk(minreal(W_1 * s / (s + 0.01*w_c) * s / (s + 0.01*w_c)));
figure, hold on
bodemag(1 / W_1mod, 'b')
bodemag(S, {1e-4, 1e+3}, '--g')
bodemag(S_star_zero, 'g') 
semilogx([1e-4, 1e+3], [20*log10(S_p), 20*log10(S_p)], 'r')
grid on, hold off

% W_2
W_T = 1 / W_T_inv;
figure, bodemag(W_T), grid on, hold on
bodemag(W_u_m)
W_2 = W_T;
W_2mod = tf(1 / T_p);

%% 
[Am,Bm,Cm,Dm] = linmod('generalized_plant');
M = ltisys(Am,Bm,Cm,Dm);
p_WT = abs(pole(W_T_inv));
M = sderiv(M,2,[1/p_WT(1) 1]);
M = sderiv(M,2,[1/p_WT(2) 1]);
%M = sderiv(M,2,[1/p^2, 2*zeta/p, 1]);
[gopt,G_cmod] = hinflmi(M,[1,1],0,0.01,[0,0,0]);
[Ac,Bc,Cc,Dc] = ltiss(G_cmod);
G_cmod = ss(Ac,Bc,Cc,Dc);
G_cmod = minreal(zpk(G_cmod))

G_cmod = minreal(G_cmod * (s+0.007066) / s, 1e-3)
G_cmod = minreal(G_cmod * (s+0.007204) / (s+1.882e-06), 1e-3)
G_cmod = minreal(G_cmod * (1+s/4194), 1e-3)

%% Nichols Plot
L = G_cmod * G_a * G_pn * G_f * G_s;
S = 1 / (1+L);

figure,
myngridst(T_p, S_p), hold on
nichols(L)

%% Computation of errors for requirements
S_star = minreal(S * 1 / s^2);
T = L / (1 + L);
m_S = freqresp(S, w_p);
m_T = freqresp(T, w_s);

e_r = 0 % because nu=1
e_da = 0 % because nu=1
e_dp = dcgain(S_star) * D_p0 * 2 % "* 2" because of parabolic disturbance
e_ds = abs(m_T) * a_s / G_s

t_sim = 25;
out = sim("Simulator.slx");
figure,
plot(out.y.time, out.y.data)
yline(K_d+0.14*K_d, '--r', LineWidth=0.6)
yline(K_d+0.05*K_d, '--m', LineWidth=0.6)
yline(K_d-0.05*K_d, '--m', LineWidth=0.6)
xline(2.5, '--k', LineWidth=0.6)
xline(13, '--k', LineWidth=0.6)
grid on

