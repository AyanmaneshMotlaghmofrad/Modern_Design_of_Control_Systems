...
% Plant
s = tf('s');
G_pn = K_pn / (4.5 * (1 + s/p1_n) * (1 + s/p2_n));
K_p = dcgain(G_pn);
G_s = 1;
G_a = 0.014;
D_a0 = 1.5e-3;
...
e_da = 4.5e-3;
nu_2 = 0;
S_star_zero_2 = abs(e_da / (K_p * D_a0)); % |S_star_zero| < 0.135

%S4
e_dp = 2e-3;
M_S_LF = e_dp / a_p;
M_S_LF_dB = 20*log10(M_S_LF); % M_S_LF_dB = -38.0618dB
...
%S8
s_hat = 0.12;
zeta = abs(log(s_hat)) / (sqrt(pi^2 + log(s_hat)^2)); % zeta = 0.5595

%S6
t_r = 2;
w_n_tr = 1 / (t_r * sqrt(1 - zeta^2)) * (pi - acos(zeta)); % w_n > 1.3057rad/s 

%S7
t_s5 = 8;
w_n_ts5 = log(100/5) / (t_s5 * zeta); % w_n > 0.6694rad/s 
...
w_n_min = max(w_n_tr, w_n_ts5); % w_n_min = 1.3057rad/s
...
for i = 1:n
    for j = 1:n
        for k = 1:n
            G_p = x_K(i) / (4.5 * (1 + s/x_p1(j)) * (1 + s/x_p2(k)));
...
G_cmod = minreal(G_cmod * (s+0.009693) / s, 1e-3);
G_cmod = minreal(G_cmod * (1+s/5092), 1e-3)
