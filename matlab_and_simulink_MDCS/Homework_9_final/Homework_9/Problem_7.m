...
% Plant
s = tf('s');
G_pn = K_pn / (4.5*s * (1 + s/p1_n) * (1 + s/p2_n));
K_p = dcgain(G_pn*s);
G_s = 0.5;
G_a = 0.112;
D_a0 = 1.5e-3;
...
R_0 = 1;
nu_1 = 0;
S_star_zero_1 = e_r / (K_d * R_0); % |S_star_zero| < 0.0187

%S3
e_da = 2.14;
nu_2 = 1;
S_star_zero_2 = e_da / (K_p * D_a0); % |S_star_zero| < 64.2

%S4
e_dp = 5.1e-3;
M_S_LF = e_dp / a_p;
M_S_LF_dB = 20*log10(M_S_LF); % M_S_LF_dB = -29.9310dB

%S5
e_ds = 1.6e-3;
M_T_HF = e_ds * G_s / a_s;
M_T_HF_dB = 20*log10(M_T_HF); % M_T_HF_dB = -47.9588dB

%% Transient requirements
%S8
s_hat = 0.13;
zeta = abs(log(s_hat)) / (sqrt(pi^2 + log(s_hat)^2)); % zeta = 0.5446

%S6
t_r = 1.8;
w_n_tr = 1 / (t_r * sqrt(1 - zeta^2)) * (pi - acos(zeta)); % w_n > 1.4221rad/s

%S7
t_s5 = 6;
w_n_ts5 = log(100/5) / (t_s5 * zeta); % w_n > 0.9167rad/s
...
w_n_min = max(w_n_tr, w_n_ts5); % w_n_min = 1.4221rad/s
...
for i = 1:n
    for j = 1:n
        for k = 1:n
            G_p = x_K(i) / (4.5*s * (1 + s/x_p1(j)) * (1 + s/x_p2(k)));
...
W_1mod = zpk(minreal(W_1 * s / (s + 0.001*w_c) * s / (s + 0.001*w_c)));
figure, hold on
bodemag(1 / W_1mod, 'b')
bodemag(S, {1e-4, 1e+3}, '--g')
bodemag(S_star_zero, 'g') 
...
G_cmod = minreal(G_cmod * (s+0.0009164) / s, 1e-3)
G_cmod = minreal(G_cmod * (s+0.001102) / (s+3.497e-06), 1e-3)
% G_cmod = minreal(G_cmod * (1+s/8.298), 1e-3) % This cancellation can not be
                                               % done or requirement S6 won't be satisfied
...
yline(K_d+0.13*K_d, '--r', LineWidth=0.6)
yline(K_d+0.05*K_d, '--m', LineWidth=0.6)
yline(K_d-0.05*K_d, '--m', LineWidth=0.6)
xline(1.8, '--k', LineWidth=0.6)
xline(6, '--k', LineWidth=0.6)
grid on
