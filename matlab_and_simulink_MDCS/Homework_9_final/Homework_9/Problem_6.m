...
% Plant 
s = tf('s');
G_pn = K_pn / (4.5*s * (1 + 2*zeta_pn/w_npn*s + s^2/w_npn^2));
K_p = dcgain(G_pn*s);
G_s = 3;
G_a = -0.27;
D_a0 = 8.5e-3;
...
R_0 = 1;
nu_1 = 0;
S_star_zero_1 = e_r / (K_d*R_0); % |S_star_zero| < 0.116

%S3
e_da = 1.75e-2;
nu_2 = 0;
S_star_zero_2 = e_da / (K_p*D_a0); % |S_star_zero| < 0.2316

%S4
e_dp = 0.375;
nu_3 = 1;
h_p = 2;
S_star_zero_3 = e_dp / (factorial(h_p)*D_p0); % |S_star_zero| < 62.5

%S5
e_ds = 3.3e-5;
M_T_HF = e_ds * G_s / a_s;
M_T_HF_dB = 20*log10(M_T_HF); % M_T_HF_dB = -40.0873dB

%% Transient requirements
%S8
s_hat = 0.09;
zeta = abs(log(s_hat)) / (sqrt(pi^2 + log(s_hat)^2)); % zeta = 0.6083

%S6
t_r = 2.35;
w_n_tr = 1 / (t_r * sqrt(1 - zeta^2)) * (pi - acos(zeta)); % w_n > 1.1928rad/s 

%S7
t_s5 = 8;
w_n_ts5 = log(100/5) / (t_s5 * zeta); % w_n > 0.6156rad/s 
...
w_n_min = max(w_n_tr, w_n_ts5); % w_c_min = 1.1928rad/s
...
for i = 1:n
    for j = 1:n
        for k = 1:n
            G_p = x_K(i) / (4.5*s * (1 + 2*x_zeta_p(j)/x_w_np(k)*s + s^2/x_w_np(k)^2));
...
% W_1
W_1 = zpk(1 / W_S_inv);
W_1mod = zpk(minreal(W_1 * s / (s + 0.01*w_c) * s / (s + 0.01*w_c)));
figure, hold on
bodemag(1 / W_1mod, 'b')
bodemag(S, {1e-4, 1e+3}, '--g')
bodemag(S_star_zero, 'g') 
...
% W_2
W_T = 1 / W_T_inv;
figure, bodemag(W_T), grid on, hold on
bodemag(W_u_m)
W_2 = W_T;
W_2mod = tf(1 / T_p);
...
G_cmod = minreal(G_cmod * (s+0.008358) / s, 1e-3)
G_cmod = minreal(G_cmod * (s+0.008576) / (s+4.679e-06), 1e-3)
G_cmod = minreal(G_cmod * (1+2.635/8.819*s+s^2/8.819) / (1+2.411/8.882*s+s^2/8.882), 1e-3)
% G_cmod = minreal(G_cmod * (1+s/10.62), 1e-3) % This can't be done or
                                               % requirement S5 isn't satisfied
...
e_r = 0 % because nu=1
e_da = 0 % because nu=1
e_dp = dcgain(S_star) * D_p0 * 2 % "* 2" because of parabolic disturbance
...
yline(K_d+0.09*K_d, '--r', LineWidth=0.6)
yline(K_d+0.05*K_d, '--m', LineWidth=0.6)
yline(K_d-0.05*K_d, '--m', LineWidth=0.6)
xline(2.35, '--k', LineWidth=0.6)
xline(8, '--k', LineWidth=0.6)
grid on
